...
mod lib;
use crate::atomic_util::{add_usize, get_usize, set_usize};
use crate::{atomic_util, get_bool, global_var, global_var_util, libWxIkunPlus, set_bool};
// 配置常量
...
pub(crate) const APP_SOFTWARE_UPDATE_DETECTION: [&str;2] = ["https://raw.githubusercontent.com/kihlh/WxDatViewerAutoExportRust/master/version.json","https://vip.123pan.cn/1816369032/assets/WxAutoExIm/version.json"];
// pub(crate) const APP_SOFTWARE_UPDATE_DETECTION: [&str; 1] =
//     ["https://vip.123pan.cn/1816369032/assets/WxAutoExIm/debug.json"];
    ...
    if config_path.exists() {
        if let Ok(file) = std::fs::read_to_string(config_path) {
            let mut data: serde_json::Value =
                serde_json::from_str(file.as_str()).unwrap_or_else(|_| json!({}));
...
pub fn set_config<T: lib::LoadConfigValue>(config: CONFIG_KEY, value: T) -> bool {
    ...
    match config {
        CONFIG_KEY::ThreadCount => {
            set_config_usize!(CONFIG_THREAD_COUNT);
        }
        CONFIG_KEY::AutoAction => {
            set_config_bool!(CONFIG_AUTO_ACTION);
        }
        CONFIG_KEY::GlobalScan => {
            set_config_bool!(CONFIG_GLOBAL_SCAN);
        }
        CONFIG_KEY::HideSetingButton => {
            set_config_bool!(CONFIG_HIDE_SETING_BUTTON);
        }
        CONFIG_KEY::ScanAdding => {
            set_config_bool!(CONFIG_SCAN_ADDING);
        }
        CONFIG_KEY::HideSetingButton => {
            set_config_bool!(CONFIG_HIDE_SETING_BUTTON);
        }
        CONFIG_KEY::ShowObjectPreview => {
            set_config_bool!(CONFIG_SHOW_OBJECT_PREVIEW);
        }
        CONFIG_KEY::ScanLogAdding => {
            set_config_bool!(CONFIG_SCAN_LOG_ADDING);
        }
        CONFIG_KEY::Networking => {
            set_config_bool!(CONFIG_NETWORKING);
        }
        CONFIG_KEY::IgnoreMutual => {
            set_config_bool!(CONFIG_IGNORE_MUTUAL);
        }
        CONFIG_KEY::PreserveConfig => {
            set_config_bool!(CONFIG_PRESERVE_CONFIG);
        }
        CONFIG_KEY::CreateCont => {
            set_config_bool!(CONFIG_CREATE_CONT);
        }
        CONFIG_KEY::PreserveList => {
            set_config_bool!(CONFIG_PRESERVE_LIST);
        }
        CONFIG_KEY::ConsoleLog => {
            set_config_bool!(CONFIG_CONSOLE_LOG);
        }
        CONFIG_KEY::Developer => {
            set_config_bool!(CONFIG_DEVELOPER);
        }
        CONFIG_KEY::ShowMask => {
            set_config_bool!(CONFIG_SHOW_MASK);
        }
        CONFIG_KEY::DomePreview => {
            set_config_bool!(CONFIG_DOME_PREVIEW);
        }
        CONFIG_KEY::LogOutputFile => {
            set_config_bool!(CONFIG_LOG_OUTPUT_FILE);
        }
    }
...
pub fn get_config_bool(config: CONFIG_KEY) -> bool {
    ...
    match config {
        ...
        CONFIG_KEY::AutoAction => {
            get_bool!(CONFIG_AUTO_ACTION)
        }
        CONFIG_KEY::GlobalScan => {
            get_bool!(CONFIG_GLOBAL_SCAN)
        }
        CONFIG_KEY::ScanAdding => {
            get_bool!(CONFIG_SCAN_ADDING)
        }
        CONFIG_KEY::HideSetingButton => {
            get_bool!(CONFIG_HIDE_SETING_BUTTON)
        }
        CONFIG_KEY::ShowObjectPreview => {
            get_bool!(CONFIG_SHOW_OBJECT_PREVIEW)
        }
        CONFIG_KEY::ScanLogAdding => {
            get_bool!(CONFIG_SCAN_LOG_ADDING)
        }
        CONFIG_KEY::Networking => {
            get_bool!(CONFIG_NETWORKING)
        }
        CONFIG_KEY::IgnoreMutual => {
            get_bool!(CONFIG_IGNORE_MUTUAL)
        }
        CONFIG_KEY::PreserveConfig => {
            get_bool!(CONFIG_PRESERVE_CONFIG)
        }
        CONFIG_KEY::CreateCont => {
            get_bool!(CONFIG_CREATE_CONT)
        }
        CONFIG_KEY::PreserveList => {
            get_bool!(CONFIG_PRESERVE_LIST)
        }
        CONFIG_KEY::ConsoleLog => {
            get_bool!(CONFIG_CONSOLE_LOG)
        }
        CONFIG_KEY::Developer => {
            get_bool!(CONFIG_DEVELOPER)
        }
        CONFIG_KEY::ShowMask => {
            get_bool!(CONFIG_SHOW_MASK)
        }
        CONFIG_KEY::DomePreview => {
            get_bool!(CONFIG_DOME_PREVIEW)
        }
        CONFIG_KEY::LogOutputFile => {
            get_bool!(CONFIG_LOG_OUTPUT_FILE)
        }
    }
}
